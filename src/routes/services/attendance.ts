...
export async function attendanceRoutes(fastify: FastifyInstance) {
  // GET /api/services/attendance/stats - Attendance stats (company-scoped, optional date)
  fastify.get<{ Querystring: { date?: string } }>('/api/services/attendance/stats', { preHandler: [requireAuth, requireWorkifyContext] }, async (request, reply) => {
