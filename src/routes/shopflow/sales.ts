...
export async function shopflowSalesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    try {
      ...
      const customer = sale[0].customerId
        ? await sql`SELECT id, name, email, phone FROM customers WHERE id = ${sale[0].customerId} LIMIT 1`
        : []
      const user = await sql`SELECT id, name, email FROM users WHERE id = ${sale[0].userId} LIMIT 1`

      // Get items
      const saleItemsData = await sql`
        SELECT 
